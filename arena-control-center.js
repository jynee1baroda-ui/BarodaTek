...
async function fetchLiveStats() {
    try {
        ...
        if (!response.ok) {
            console.log(`⚠️  API endpoint not available (${response.status}), using mock data`);
...
async function updateLiveStats() {
    ...
    updateStatValue('activeUsers', stats.activeUsers);
    updateStatValue('pageViews', stats.pageViews);
    updateStatValue('apiRequests', stats.apiRequests);
    updateStatValue('uptime', stats.uptime.toFixed(1) + '%');
    updateStatValue('responseTime', stats.responseTime + 'ms');
    updateStatValue('portNumber', stats.port);
    updateStatValue('portStatus', stats.status.toUpperCase());
    
    // Update charts
    updateChart('usersChart', stats.activeUsers);
    updateChart('viewsChart', stats.pageViews);
    updateChart('requestsChart', stats.apiRequests);
    updateChart('responseChart', stats.responseTime);
